...
class FileOpener:
    ...
    def open_file(self, file_type: str = "html", datafile: str = "") -> None:
        ...
        """
        cmd = (
            self.config_service.get_file_command(file_type)
            if hasattr(self.config_service, "get_file_command")
            else None
        )
        if not cmd:
    ...
    def _open_with_startfile(self, datafile: str) -> None:
        ...
        if not hasattr(os, "startfile"):
            raise EnvironmentError(
                "os.startfile is Windows-only. " "Change process option in Settings to 'subprocess.popen'"
            )

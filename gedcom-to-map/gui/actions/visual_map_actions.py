...
from const import (
    GLOBAL_GEO_CACHE_FILENAME,
    FILE_ALT_PLACE_FILENAME_SUFFIX,
    FILE_GEOCACHE_FILENAME_SUFFIX,
    GEO_CONFIG_FILENAME,
)
...
class VisualMapActions:
            ...
            for h in people:
                try:
                    if referenced.exists(people[h].xref_id):
                        ...
                        trace.write(
                            f"{people[h].xref_id}\t{people[h].name}\t{ref_year}\t{where}\t{location}\t"
                            + "\t".join(personpath)
                            + "\n"
                        )
                except Exception:
    ...
    def doTraceTo(
        self, svc_config: IConfig, svc_state: IState, svc_progress: IProgressTracker, ToID: Person

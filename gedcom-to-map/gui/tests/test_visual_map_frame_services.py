...
class TestOnInfoServiceIntegration:
    ...
    def test_oninfo_uses_correct_selectedpeople_attribute(self):
        ...
        assert hasattr(state, 'selectedpeople'), \
            "GVState must have 'selectedpeople' attribute for OnInfo()"
        assert not hasattr(state, 'selected_people'), \
            "GVState should not have 'selected_people' (note underscore)"
    ...
    def test_oninfo_required_attributes_exist(self):
        ...
        for attr in required_attrs:
            assert hasattr(state, attr), \
                f"GVState missing '{attr}' attribute required by OnInfo()"

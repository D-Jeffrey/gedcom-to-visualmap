...
class PeopleListCtrl(wx.ListCtrl, listmix.ListCtrlAutoWidthMixin, listmix.ColumnSorterMixin):
    def __init__(
        ...
        **kw,
        ...
        self.id = (
            VisualGedcomIds(svc_config=self.svc_config)
            if VisualGedcomIds
            else type(
                "IdStub", (), {"GetColor": lambda *_a, **_k: wx.WHITE, "SmallUpArrow": None, "SmallDnArrow": None}
            )()
        )
        self.active = False
    ...
    def PopulateList(self, people, mainperson, loading):
        ...
        try:
            self.GridOnlyFamily = (
                self.svc_config.get("GridView")
                if (self.svc_config is not None and hasattr(self.svc_config, "get"))
                else False
            )
        except Exception:
            self.GridOnlyFamily = False
                                    ...
                                    if issues:
                                        ...
                                        if self.color_manager:
                                            self.SetItemBackgroundColour(
                                                index, self.color_manager.get_color("ANCESTOR")
                                            )

...
class KmlExporter:
                    ...
                    if self.svc_state.Referenced.exists(tag):
                        ...
                        for ripout in re.findall(replaceit, placemark.description):
                            if ripout:
                                ...
                                placemark.description = placemark.description.replace(
                                    ripout[0] + ripout[1] + ripout[2], ripout[1]
                                )
    ...
    def _setup_kml_and_styles(
        self, colorA: str, colorB: str, marktype: str, foldermode: bool
    ) -> tuple[simplekml.Kml, simplekml.Style, simplekml.Style]:
        """
        ...
        if self.kml:
            ...
            kml = simplekml.Kml()
            inputfile = (
                os.path.basename(self.svc_config.get("GEDCOMinput"))
                if self.svc_config.get("GEDCOMinput")
                else "Unknown"
            )
            descript = (
                f"Family tree generated for using {inputfile}<br>"
                f"{self.svc_config.get('Name')} ({self.svc_config.get('Main')}) as starting person"
            )
            descript += (
                f"<br>Marker types are "
                f"{'Birth' if self.svc_config.get('BornMark') else ''} "
                f"{'Death' if self.svc_config.get('DieMark') else ''}"
            )
            kmloptions = []
    ...
    def _format_children_links(self, line: Line, foldermode: bool) -> str:
        ...
        people = self.svc_state.people
        if self.svc_config.get("UseBalloonFlyto"):
            family_links = [f"<a href=#{child[1:-1]};balloonFlyto>{people[child].name}</a>" for child in children]
    ...
    def _add_life_line(
        self, line: Line, desend: str, event: str, linage: str, familyLinage: str, foldermode: bool, kml: simplekml.Kml
    ) -> None:
        """
        ...
        kml_line.linestyle.width = max(int(self.max_line_weight / math.exp(0.5 * min(line.prof, 100))), 0.1)
        kml_line.extrude = 1
        ...
        _log.debug(
            f"    line    {line.name} ({line.fromlocation.lon}, {line.fromlocation.lat}) ({line.tolocation.lon}, {line.tolocation.lat})"
        )

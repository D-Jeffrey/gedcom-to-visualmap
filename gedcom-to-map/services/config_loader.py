...
class LoggingConfigApplicator:
    ...
    @classmethod
    def cleanup_stale_loggers(cls, config: configparser.ConfigParser, valid_logger_names: list) -> list:
        ...
        if stale_loggers:
            _log.info("Removed %d stale logger(s): %s", len(stale_loggers), ", ".join(stale_loggers))

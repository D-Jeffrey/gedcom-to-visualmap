...
from services.config_loader import YAMLConfigLoader, INIConfigLoader, LoggingConfigApplicator
...
class GVConfig(IConfig):
    ...
    def __init__(
        ...
        logging_applicator: Optional[LoggingConfigApplicator] = None,
    ) -> None:
        """
    ...
    def create_for_testing(
        cls, yaml_data: Dict[str, Any], ini_config: Optional[configparser.ConfigParser] = None
    ) -> "GVConfig":
        ...
        return cls(yaml_loader=MockYAMLLoader, ini_loader=MockINILoader("/tmp/test_settings.ini"))
    ...
    def _cleanup_and_load_logging_section(self) -> None:
        ...
        stale_loggers = self._logging_applicator.cleanup_stale_loggers(self.gvConfig, logging_keys)
    ...
    def savesettings(self) -> None:
        ...
        try:
            ...
            self.gvConfig["Core"]["OutputFile"] = os.path.join(
                getattr(self, "resultpath", ""), getattr(self, "ResultFile", "")
            )
    ...
    def _get_option_sections(self) -> list[str]:
        ...
        for key, value in self.options.items():
            if isinstance(value, dict) and any(
                isinstance(v, dict) and "type" in v and "default" in v for v in value.values()
            ):
                sections.append(key)
        return sections
    ...
    def _initialize_file_commands(self) -> None:
        ...
        if not platform_defaults:
            ...
            _log.warning(
                "No file command defaults found for platform '%s' in %s. " "Available platforms: %s",
                os_name,
                self.GEDCOM_OPTIONS_FILE,
                available,
            )
            return

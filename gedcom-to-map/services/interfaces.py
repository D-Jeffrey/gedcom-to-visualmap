...
class IAppHooks(Protocol):
    ...
    def report_step(self, state: str = None, info: str = None, target: int = -1, 
                    reset_counter: bool = False, plus_step: int = 1, set_counter: int = None) -> None:
...
class ServiceContainer:
    ...
    def register(self, interface: Type[T], implementation: T, singleton: bool = True) -> None:
        ...
        _log.debug("Registered service: %s -> %s (singleton=%s)", 
                  interface.__name__, type(implementation).__name__, singleton)
